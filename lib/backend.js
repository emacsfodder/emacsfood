 ...
 * EmacsFood-api(7) manpage, under Packages.
 */
...
/**
 * Initialize a EmacsFood backend. Note that this function may actually block
...
/**
 * A class representing a EmacsFood backend. This constructor may actually block
 * for a nontrivial amount of time.
 *
 * @param {string} hostname The name of the server on which this is running.
 *   Used for emails.
...
var Backend = function(hostname, callback) {
    ...
    step(
        function() {
            self.email = "EmacsFood Server <" + (process.env["USER"] || "") +
                "@" + hostname + ">";
            self.mongoServer_ = new mongodb.Server(
                  'localhost', mongodb.Connection.DEFAULT_PORT, {});
            new mongodb.Db('emacsfood', self.mongoServer_).open(this);
        },
        function(err, db) {
            if (err) throw err;
            self.db_ = db;
...
/**
 * The email address used as the From address for emails sent by EmacsFood. At
 * time of writing, these are only password-reset email.
 *
 * Defaults to `EmacsFood Server <$USER@hostname>`.
 *
 * @type {String}
 */
...
Backend.prototype.read_ = function(filename, callback) {
    var tempfile;
    step(
        function() {util.run('tempfile', ['-p', 'meatpack '], this)},
        function(err, tempfile_) {
            if (err) throw err;
            tempfile = tempfile_.replace(/\n$/, '');
            util.run('mongofiles',
                     ['get', '-d', 'meatpack ', '-l', tempfile, filename],
                     this);
        },
        function(err) {
            if (err) throw err;
...
Backend.prototype.write_ = function(filename, data, callback) {
    ...
    step(
        ...
        function(err) {
            if (err) throw err;
            util.run('mongofiles',
                     ['put', '-d', 'meatpack ', '-l', tempfile, filename],
                     this);
        },
        function(err) {
            if (tempfile) fs.unlink(tempfile) // ignore errors from this
...
Backend.prototype.rm_ = function(filename, callback) {
    util.run('mongofiles', ['delete', '-d', 'meatpack ', filename], callback);
};
 ...
 * @param {function(Error=)} callback
 */
Backend.prototype.resetPassword = function(email, callback) {
    var user, password;
    var self = this;
    step(
        function() {
            streamUtil.all(stream(self.users_, {email: email.toLowerCase()}),
                           this.group()());
            util.run("openssl", ["rand", "-base64", "10"], this.group()());
        },
        function(err, users_, password_) {
            if (err) throw err;
            var users = users_[0];
            if (!users || users.length == 0) throw new exports.InputError(
                "No users exist with email " + email);
            password = password_[0].replace(/\n$/, '');
            if (users.length == 1) {
                user = users[0];
            ...
            self.initiateMerge(users, password, function(err) {
              if (err) return callback(err);
              callback(new exports.InputError(
                  "Multiple users exist with email " + email + ". Email for " +
                    "merging users sent."));
            });
        },
        function(err) {
            ...
            new Email({
                to: user.email,
                from: self.email,
                subject: "EmacsFood password reset",
                body: "Temporary password for account " + user.name + " : " +
                  password
            }).send(this);
        }, callback);
};
...
/**
 * Emails a user with multiple accounts to begin the merging process.
 *
 * @param {Array.<Object>} users The user accounts to merge.
 * @param {string} token The security token to verify that the account owner is
 *     doing the merging.
 * @param {function(Error=)} callback
 */
Backend.prototype.initiateMerge = function(users, token, callback) {
  var self = this;
  step(
      function() {
          var nexts = this.group();
          users.forEach(function(user) {
              user.mergeToken = token;
              self.saveUser(user, self.checkErrorCb(nexts()));
          });
      },
      function(err, _) {
          if (err) throw err;
          new Email({
              to: users[0].email,
              from: self.email,
              subject: "Merging your EmacsFood accounts",
              body: "You're receiving this email because you have multiple " +
                "EmacsFood accounts registered to this email address that " +
                "need to be merged together. Please choose your preferred " +
                "account and follow the link:\n\n" + users.map(function(user) {
                  return "* " + user.name + ": http://" + self.hostname +
                      "/profiles/" + user.name + "/merge?t=" +
...
/**
 * Merges all accounts with the same email as a given user into that user's
 * account.
 *
 * @param {string} name The name of the primary account.
 * @param {string} token The token to verify that this merge is allowed.
 * @param {function(Error=, Object)} callback Passed the merged user, without
 *     any extra packages that have been merged in.
 */
Backend.prototype.performMerge = function(name, token, callback) {
  var user, packageNames, removedNames;
  var self = this;
  step(
      function() {
          self.users_.findOne({_name: self.key(name)},
                              self.checkErrorCb(this));
      },
      function(err, user_) {
          if (err) throw err;
          user = user_;
          if (user.mergeToken != token) throw new exports.InputError(
              "Security token doesn't match.");
          streamUtil.all(stream(self.users_, {email: user.email}),
                         self.checkErrorCb(this));
      },
      function(err, users_) {
          if (err) throw err;
          var removedPackages = {};
          var removedUsers = users_.filter(
              function(u) { return u._name != user.name; });

          removedUsers.forEach(function(u) {
              u.packages.forEach(function(p) { removedPackages[p] = true; });
          });
          packageNames = _.keys(removedPackages);
          removedNames = removedUsers.map(function(u) { return u._name; });

          var update = {$set: {}, $unset: {}};
          update.$set['owners.' + user.name] = user.email;
          removedNames.forEach(
              function(u) { update.$unset['owners.' + u] = 1; });

          self.packages_.update({_name: {$in: packageNames}}, update,
                                {multi: true}, self.checkErrorCb(this));
      },
      function(err) {
          if (err) throw err;
          self.users_.remove({_name: {$in: removedNames}}, {multi: true},
                             self.checkErrorCb(this.group()()));
          self.users_.update({_name: user._name},
                             {$addToSet: {packages: {$each: packageNames}}},
                             self.checkErrorCb(this.group()()));
      },
      function(err) {
          if (err) throw err;

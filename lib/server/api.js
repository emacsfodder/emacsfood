...
/**
 * The API for programatically interacting with the EmacsFood backend. This
 * includes both the standard ELPA API and the EmacsFood API for user-uploaded
 * packages.
 */
...
/**
 * Create the EmacsFood API server, for handling the ELPA API and EmacsFood's
 * own HTTP API.
 * @param {backend.Backend} be The EmacsFood backend.
...
exports.create = function(be) {
    ...
    app.get('/packages/archive-contents', function(req, res, next) {
        var pkgStream = be.packageStream();
        var first = true;

        pkgStream.on('data', function(pkg) {
            var version = pkg._latestVersion;
            if (!version) return;
            _.each(version.requires, function(req) {
            ...
            step(
                function() {
                    res.partial("archive-contents.ejs", {
                      locals: {pkg: version},
                      layout: false
                    }, this);
                },
     ...
     *
     * All responses to EmacsFood-specific API calls can be either JSON or
     ...
     * package metadata (as describd in the EmacsFood-api(7) manpage).
     */
        ...
        step(
            function() {
                if (!req.body.name) {
                    ...
                    be.loadUserWithToken(req.body.name, req.body.token, this);
                }
            },
            function(err, user) {
                if (err) throw err;

                var ext = req.body['package'].filename.match(/\.([^.]+)$/);
                if (!ext) {
                    throw new base.HttpError(
                          "Couldn't determine file extension for " +
                              req.body['package'].filename,
                          400);
                }
                ext = ext[1];

                if (ext === "tar") {
                    be.saveTarFile(req.body['package'].path, user, this);
                } else if (ext === "el") {
                    be.saveElispFile(req.body['package'].path, user, this);
                } else {
    ...
    /**
     * Removes a specific version of a package. This should have `name` and
     * `token` parameters.
     */
    app.del(/^\/v1\/packages\/([^\/]+)\/([0-9]+(?:\.[0-9]+)*)\/?$/, function(
          req, res, next) {
        var name = req.params[0];
        var version = _.map(req.params[1].split('.'), Number);
        step(
            ...
            function(err, owner) {
                if (err) throw err;
                be.checkPackageOwner(owner, name, this);
            },
            function(err) {
                if (err) throw err;
                be.removePackageVersion(name, version, this);
            },
            function(err) {
                if (err) throw err;
                res.send({message: "Successfully removed " + name +
                          ", version " + version.join('.')});
            }, next);
    });

    /**
     * Removes all versions of a package. This should have `name` and `token`
     * parameters.
     */
    app.del('/v1/packages/:package', function(req, res, next) {
        var name = req.requiredParam('package');
        step(
            function() {
                be.loadUserWithToken(req.requiredParam('name'),
                                     req.requiredParam('token'),
     ...
     * metadata (as describd in the EmacsFood-api(7) manpage).
     */
    ...
    app.post('/v1/users/reset', function(req, res, next) {
        step(
            function() {
                be.resetPassword(req.requiredParam('email'), this);
            },
            function(err) {
                if (err) throw err;

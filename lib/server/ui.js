...
/**
 * The web frontend for EmacsFood.
 ...
 * EmacsFood-specific functionality.
 */
function extensions(req, res, next) {
    ...
    res.render = function(view, options, fn, parent, sub) {
        ...
        options = options || {};
        if (!options.isLayout && !options.isPartial) {
          options.layout = 'layout.jade';
        }
        options.locals = _.extend({
            mainClass: view.replace('/', '-').replace(/\.jade$/, ''),
            currentUrl: req.url
        }, options.locals || {});
        return oldRender(view, options, fn, parent, sub);
    };
...
/**
 * Create the EmacsFood UI server, for handling the web UI.
...
exports.create = function(be) {
    ...
    app.helpers({
       ...
       */
      packageDownloadPath: function(version) {
        ...
        step(
            ...
            function(err, packages) {
                ...
                res.render('index.jade', {
                    locals: {packages: packages || [], util: util}
                });
            }, next);
    });
    ...
    app.get('/forgot', function(req, res, next) {
      step(function() {res.render('forgot.jade')}, next);
    });
    ...
    app.post('/forgot', function(req, res, next) {
      step(
          function() {
              res.errorView = 'forgot.jade';
              be.resetPassword(req.requiredParam('email'), this);
          },
          function(err) {
              if (err) throw err;
              req.flash('info', 'Email sent with temporary password.');
              res.redirect('/login', 301);
          }, next);
    });

    /**
     * Merges multiple users with the same email.
     */
    app.get('/profiles/:name/merge', function(req, res, next) {
      var name;
      step(
          function() {
              name = req.requiredParam('name');
              be.performMerge(name, req.requiredParam('t'), this);
          },
          function(err, user) {
              if (err) throw err;
              be.resetPassword(user.email, this);
          },
          function(err) {
              if (err) throw err;
              req.flash('info', 'Email sent with temporary password for ' +
                        'account ' + name + '.');
              res.redirect('/login', 301);
          }, next);
    });
        ...
        step(
            ...
            function (err, packages) {
                ...
                res.render('packages/index.jade', {
                    locals: {
                        query: query || null, // undefined throws error
                        page: page,
        ...
        step(
            ...
            function(err, pkg_, version_) {
                ...
                if (!pkg_) throw new base.HttpError(
                      ...
                      (req.params[1] ? 'version "' + req.params[1] + '" ' : '') +
                      'doesn\'t exist', 404);
    ...
    /**
     * Action for deleting a package version.
     */
    app.del(/^\/packages\/([^\/]+)\/([0-9]+(?:\.[0-9]+)*)\/?$/, function(
          req, res, next) {
            ...
            function(err, user) {
                ...
                if (!_.include(user.packages, be.key(pkgName))) {
                    throw new base.UserError(
                          'You don\'t have permission to delete versions for ' +
                          'package "' + pkgName + '"');
                }
                be.removePackageVersion(pkgName, version, this);
            },
            function(err) {
                if (err) throw err;
                req.flash('info',
                          'Successfully deleted "' + pkgName + '" version ' +
                          version.join('.'));
                res.redirect('/');
            }, next);
    });
    ...
    app.post('/packages', function(req, res, next) {
        ...
        step(
            ...
            function(err, pkg) {
                if (err) throw err;
                res.redirect('/packages/' + pkg.name + '/' +
            ...
            function(err, user_) {
                ...
                if (!user_) {
                    throw new base.HttpError(
                        'User "' + req.param('name') + '" doesn\'t exist', 404);
                }
                user = user_;
                stream.all(

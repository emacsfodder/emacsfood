...
/**
 * Create a EmacsFood server. This is a standard Express application, with
 * routes and configuration set up for EmacsFood. To start it, just call
 * `.create().listen()`.
...
exports.create = function(opt_hostname, callback) {
    ...
    /**
     * The EmacsFood backend. This will be set once the caller receives the app.
    ...
    app.addListener('listening', function(err) {
        console.log("EmacsFood's cooking on " + app.hostname + ":" + app.address().port);
    });
